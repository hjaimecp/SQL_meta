...
st.markdown(
    ...
    <style>
        ...
        .quote-box {
            border-left:4px solid #98A2B3;background:#F8FAFC;padding:12px 14px;
            margin:8px 0;border-radius:0 10px 10px 0;font-size:.94rem;
        }
        .case-card {
        ...
        .hypothesis {
            border:1px solid #E5E7EB;border-radius:14px;padding:16px 18px;
            background:#FFFFFF;min-height:170px;
        }
    </style>
    """,
...
def filter_lost_cases(df, fields):
    ...
    fase_perdida = fase.str.contains(
        r"Cierre\s*Perdido|Perdido",
        case=False,
        na=False,
        regex=True,
    )

    estatus_perdido = estatus.str.contains(
        r"Perdido|Cierre\s*Perdido",
        case=False,
        na=False,
        regex=True,
    )

    motivo_descartado = motivo.str.contains(
        "Descartado",
        case=False,
        na=False,
    )

    perdida = fase_perdida | estatus_perdido | motivo_descartado

    excluir = (
        fase.str.contains(
            r"Cierre\s*Logrado|Análisis de Necesidades|Analisis de Necesidades|Calificación|Calificacion",
            case=False,
            na=False,
            regex=True,
        )
    ...
    result = df[perdida & ~excluir].copy()
...
def classify_row(row, fields):
    ...
    parts += [clean_text(row.get("sales_notes_text","")), clean_text(row.get("conversation_text_all",""))]
    all_text = "\n".join(parts)

    if text_contains(all_text, ["liga de pago","link de pago","iniciaria con el plan","iniciaría con el plan","procedia para contratar","procedía para contratar"]):
        return "Casi cierre"
    if text_contains(all_text, ["se queda con su proveedor actual","se quedara con otra plataforma","se quedará con otra plataforma","se decidio por otro servicio","se decidió por otro servicio","competencia"]):
        return "Proveedor actual / competencia"
    if text_contains(all_text, ["comprobante de domicilio","planes moviles","planes móviles","telefonia movil con internet","telefonía móvil con internet","linea telefonica unicamente","línea telefónica únicamente","quiere utilizar su numero movil como principal","quiere utilizar su número móvil como principal"]):
        return "Mismatch explícito"
    if text_contains(all_text, ["sin documentos de validacion","sin documentos de validación","aun no tiene los documentos","aún no tiene los documentos"]):
        return "Requisito / documentación"
    if text_contains(all_text, ["curioseando","solo estoy investigando","sólo estoy investigando","no es un proyecto","proyecto no listo","falta de definicion de presupuesto","falta de definición de presupuesto","sin aprobacion final","sin aprobación final"]):
        return "Proyecto inmaduro / exploración"
    if text_contains(all_text, ["respondiera en automatico","respondiera en automático","automatizacion","automatización","agente virtual","inteligencia artificial"]) and text_contains(all_text, ["no responde","sin respuesta","dejo de contestar","dejó de contestar"]):
        return "Necesidad ampliada + no respuesta"

    has_advance = text_contains(all_text, ["propuesta enviada","se envio propuesta","se envió propuesta","cotizacion enviada","cotización enviada","videollamada","demo","demostracion","demostración","propuesta comercial"])
    has_no_response = text_contains(all_text, ["no responde","sin respuesta","dejo de contestar","dejó de contestar","no se conecto","no se conectó","no asistio","no asistió","cuelga la llamada","corta la llamada"])
...
st.caption(
    "La única referencia individual visible es deal_id. "
    "Nombres, empresas, correos, teléfonos, sitios web, URLs y otros identificadores se redactan."
)
...
ad_values = sorted(lost["ad_value_display"].unique().tolist())
sizes = sorted(lost["tamano_display"].unique().tolist())
states = sorted(lost["estado_display"].unique().tolist())
classes = [c for c in CATEGORY_COLORS if c in lost["clasificacion"].unique()]
...
f_class = st.sidebar.multiselect("Clasificación", classes, default=classes)
...
with tabs[0]:
    if view.empty:
        ...
        st.info("No se muestran nombres, contactos, empresas, correos, teléfonos ni sitios web.")
...
with tabs[1]:
    ...
    if not view.empty:
        ...
        with c1: metric_card("Clasificación",row["clasificacion"],"regla conservadora",color)
        with c2: metric_card("Tamaño",row["tamano_display"],row["estado_display"],COLORS["orange"])
        with c3: metric_card("Cotización",row["cotizacion_display"],row["presupuesto_display"],COLORS["blue"])
        with c4: metric_card("Deal",row["deal_id"],"único identificador visible",COLORS["purple"])
...
with tabs[2]:
    ...
    st.caption("No son conclusiones causales; son señales para validar.")
    ...
    c1,c2 = st.columns(2)
    with c1:
        st.markdown(f"""<div class="hypothesis"><b><span class="dot" style="background:{COLORS['red']};"></span>H1 · Fuga después de avance comercial</b><br><br><b>{ghost_n+near_n}/{total}</b> casos están en ghosting post-avance o casi cierre.<br><br><span class="small-note">Localiza la fuga; no explica la causa.</span></div>""", unsafe_allow_html=True)
    with c2:
        st.markdown(f"""<div class="hypothesis"><b><span class="dot" style="background:{COLORS['orange']};"></span>H2 · Parte del copy admite interpretaciones distintas</b><br><br><b>{mismatch_n}/{total}</b> casos tienen mismatch explícito.<br><br><span class="small-note">Validar con fit real después de la primera llamada.</span></div>""", unsafe_allow_html=True)

    c3,c4 = st.columns(2)
    with c3:
        st.markdown("""<div class="hypothesis"><b><span class="dot" style="background:#3B6EA8;"></span>H3 · Cotización enviada puede medir actividad, no intención</b><br><br>Separar: enviada → revisada → respondió → negoció → aceptó → pagó.</div>""", unsafe_allow_html=True)
    with c4:
        st.markdown(f"""<div class="hypothesis"><b><span class="dot" style="background:{COLORS['yellow']};"></span>H4 · Concentración en microempresas</b><br><br><b>{micro_n}/{total}</b> descartados son microempresas.<br><br><span class="small-note">No implica peor tasa de cierre sin grupo comparativo.</span></div>""", unsafe_allow_html=True)

    st.warning("El dashboard no atribuye causalidad a precio, vendedor, tamaño, estado o anuncio sin evidencia explícita.")

st.markdown("---")
st.caption("Fuente: Google Sheets público · cruce por deal_id · PII redactada antes de renderizar.")
